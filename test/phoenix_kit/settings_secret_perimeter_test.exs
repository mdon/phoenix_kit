defmodule PhoenixKit.SettingsSecretPerimeterTest do
  @moduledoc """
  Moving keys from `@restricted_setting_keys` to `@public_setting_keys` and
  watching the round-trip tests go red only proves the tests catch a
  SHIFTED boundary. It does not prove they would have caught the actual
  historical failure — a key
  ...
  describe "the old get_defaults/0 partition invariant cannot see a key outside get_defaults/0" do
    test "a secret-shaped key absent from all three lists is written unencrypted, and the invariant still passes" do
      probe_key = "perimeter_probe_totally_secret_key"
  ...
  describe "core-owned secret-key perimeter guard" do
    ...
    # restricted_setting_keys/0 — every case this scan can see has already
    # been closed; this test exists to keep it closed, not to find something
    # new right now.
    ...
    # via a pre-mutation copy — never `git checkout`) was verified manually
    # before this fixture-based version was committed; this test is what
    # stays in the suite afterward, small and fast enough to run on every
    # future PR.
