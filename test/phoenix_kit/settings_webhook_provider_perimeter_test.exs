defmodule PhoenixKit.SettingsWebhookProviderPerimeterTest do
  @moduledoc """
  `billing_paypal_webhook_secret` was missing from `@restricted_setting_keys`
  despite the earlier manual audit and the static perimeter guard in
  `settings_secret_perimeter_test.exs`, because
  `PhoenixKitBilling.Web.WebhookController.get_webhook_secret/1` builds the
  key by STRING INTERPOLATION — `"billing_\#{provider}_webhook_secret"` —
  never as a literal. Both the manual grep and
  `SecretKeyPerimeter.scan_settings_literals/1` look for literal string
  arguments; a call site with a variable is invisible to either, regardless
  of how constant that variable's value turns out to be at every call site
  ...
  # local core); unset falls back to the common sibling-checkout layout.
  #
  # phoenix_kit_billing is a separate package with no place in a plain clone
  # of core (a fresh checkout, or upstream CI, has no sibling checkout at
  # all) — so the describe block below skips this test outright when the
  # directory can't be found, the same way the suite already excludes
  # integration tests when no database is reachable. When the directory IS
  # present, there is deliberately no further leniency: an empty or
  # unexpected scan result fails loudly via the floor assertions below
  # rather than passing having checked nothing (a moved/renamed directory
  # inside an otherwise-present checkout must still be caught).
  defp billing_repo_path do
    System.get_env("PHOENIX_KIT_BILLING_PATH") || "/www/phoenix_kit_billing"
  end

  describe "webhook-secret key family resolved from real call sites" do
    setup do
      ...
      if File.dir?(root) do
        ...
        {:skip,
         "phoenix_kit_billing checkout not found at #{root} (set PHOENIX_KIT_BILLING_PATH to run this test)"}
      end
    end

    test "every provider handle_webhook/3 is called with has a restricted billing_<provider>_webhook_secret",
         %{root: root} do
  ...
  describe "webhook-secret scan mechanism" do
    ...
    # Deliberately outside the describe block above: this does not need the
    # real phoenix_kit_billing checkout and must keep running even when it
    # is absent.
    test "the call-site scan is not vacuous: it finds a planted handle_webhook/3 call" do
      tmp_root =
        Path.join(
          System.tmp_dir!(),
          "webhook_fixture_#{System.unique_integer([:positive])}"
        )
