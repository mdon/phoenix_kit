defmodule PhoenixKit.SettingsJsonNilFalseTest do
  @moduledoc """
  `get_json_settings_cached/2` and its miss-fill writer,
  `fill_missing_json_settings/1`, used `||` to fall back to a default
  whenever the looked-up value was falsy — the same class of bug the
  miss-fill sentinel fix addressed one layer up, but in two different
  spots:
  ...
  Both are fixed in this change (`Map.fetch`/`case` instead of `||`), and
  this file only tells the fixed code apart from the broken code once the
  miss-fill sentinel fix is also in place — on unmodified upstream, every
  read here takes the `:noproc`-absorbed path regardless of these two
  points, so it passes there too and isn't a regression guard against that
  baseline on its own.
  ...
  defp probe_key, do: "json_nil_false_probe_#{System.unique_integer([:positive])}"
