defmodule PhoenixKitWeb.Live.Settings.AuthorizationOAuthSecretValidationTest do
  @moduledoc """
  `phoenix_kit_settings` had no format check on `:value` at all, so a
  9-character Google OAuth secret saved silently and stayed live for four
  days. This covers the save-time gate added in
  ...
  requires a live database (`PGHOST`/`PGDATABASE`, see AGENTS.md).
  Written and reviewed against the same pattern as
  `authorization_secret_leak_test.exs` in this directory.
  """
  ...
  test "an already-stored short legacy secret does not block saving an unrelated field", %{
    ...
    view
    |> form("#authorization_settings_form", %{
      "settings" => %{"auth_background_color" => "#112233"}
    })
    |> render_submit()
