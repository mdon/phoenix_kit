defmodule PhoenixKit.Migrations.Postgres.V175 do
  @moduledoc """
  V175: validate existing NOT VALID foreign keys. Never deletes a row.
  ...
  def down(%{prefix: prefix} = _opts) do
    ...
    execute("COMMENT ON TABLE #{prefix_table_name("phoenix_kit", prefix)} IS '174'")
  end
  ...
  def attempt_validate(
    ...
    case Probe.orphan_count(repo, prefix, table_str, uuid_fk, ref_table, ref_col) do
      0 ->
        ...
        EXCEPTION
          WHEN OTHERS THEN
            RAISE WARNING 'PhoenixKit V175: VALIDATE CONSTRAINT #{conname} failed: % (SQLSTATE %)',
              SQLERRM, SQLSTATE;
        END $$;
        """)
  ...
  defp report_still_not_valid(labels) do
    IO.warn(
      "PhoenixKit V175 SUMMARY: #{length(labels)} constraint(s) are NOT VALID and could NOT be " <>
  ...
  defp fk_shape_present(repo, table_str, uuid_fk, ref_table, ref_col, escaped_prefix) do
    ...
    case repo.query(query, [], log: false) do
      ...
      # there.
      other -> raise "PhoenixKit V175: could not probe FKs on #{table_str}: #{inspect(other)}"
    end
  end
