defmodule PhoenixKit.Migrations.Postgres.V07 do
  ...
  def up(%{prefix: prefix} = _opts) do
    ...
    create_if_not_exists table(:phoenix_kit_email_logs, prefix: prefix) do
      add :message_id, :string, null: false             # Unique ID from email provider
      add :to, :string, null: false                     # Recipient email
      add :from, :string, null: false                   # Sender email
      add :subject, :string, null: true                 # Email subject
      add :headers, :map, null: true, default: %{}      # JSONB headers without duplication
      add :body_preview, :text, null: true              # First 500+ characters preview
      add :body_full, :text, null: true                 # Full email body (optional)
      add :template_name, :string, null: true           # Email template identifier
      add :campaign_id, :string, null: true             # Campaign/group identifier
      add :attachments_count, :integer, null: false, default: 0  # Number of attachments
      add :size_bytes, :integer, null: true             # Email size in bytes
      add :retry_count, :integer, null: false, default: 0        # Send retry attempts
      add :error_message, :text, null: true             # Error message if failed
      add :status, :string, null: false, default: "sent" # sent, delivered, bounced, opened, clicked, failed
      add :sent_at, :utc_datetime_usec, null: false, default: fragment("NOW()") # Send timestamp
      add :delivered_at, :utc_datetime_usec, null: true  # Delivery timestamp
      add :configuration_set, :string, null: true        # AWS SES configuration set
      add :message_tags, :map, null: true, default: %{}  # JSONB tags for grouping
      add :provider, :string, null: false, default: "unknown" # aws_ses, smtp, local, etc.
      add :user_id, :integer, null: true                 # FK to users for auth integration
    end

    # Create email events table
    create_if_not_exists table(:phoenix_kit_email_events, prefix: prefix) do
      add :email_log_id, :integer, null: false          # FK to email_logs
      add :event_type, :string, null: false             # open, click, bounce, complaint, delivery, send
      add :event_data, :map, null: true, default: %{}   # JSONB event-specific data
      add :occurred_at, :utc_datetime_usec, null: false, default: fragment("NOW()") # Event timestamp
      add :ip_address, :string, null: true              # IP for open/click events
      add :user_agent, :string, null: true              # User agent for open/click
      add :geo_location, :map, null: true, default: %{} # JSONB geo data (country, region, city)
      add :link_url, :string, null: true                # URL for click events
      add :bounce_type, :string, null: true             # hard, soft for bounce events
      add :complaint_type, :string, null: true          # abuse, auth-failure, fraud, etc.
    end
