defmodule PhoenixKitWeb.Live.EmailTracking.EmailDetailsLive do
  ...
  This LiveView is mounted at `/phoenix_kit/admin/email-logs/:id` and requires
      ...
      case Integer.parse(id) do
        ...
        _ ->
          {:ok,
           ...
           |> redirect(to: ~p"/phoenix_kit/admin/email-logs")}
      end
    else
      {:ok,
       socket
  ...
  def handle_event("view_related", %{"campaign_id" => campaign_id}, socket) do
    {:noreply,
     socket
     |> redirect(to: ~p"/phoenix_kit/admin/email-logs?campaign_id=#{campaign_id}")}
  end
  ...
  def handle_event("view_related", %{"template_name" => template_name}, socket) do
    {:noreply,
     socket
     |> redirect(to: ~p"/phoenix_kit/admin/email-logs?template_name=#{template_name}")}
  end
  ...
  def render(assigns) do
    ...
    <div class="email-details-container">
      ...
      <div class="breadcrumbs text-sm mb-6">
        <ul>
          <li><.link navigate={~p"/phoenix_kit/admin/dashboard"}>Admin</.link></li>
          <li><.link navigate={~p"/phoenix_kit/admin/email-logs"}>Email Logs</.link></li>
      ...
      <% else %>
          ...
          <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-6">
            <div>
              ...
              <div class="flex items-center gap-4 text-sm text-gray-600">
                ...
                <span>•</span>
                <span>Sent: {UtilsDate.format_datetime_with_user_format(@email_log.sent_at)}</span>
              </div>
            </div>
            ...
            <div class="lg:col-span-2 space-y-6">
                  ...
                  <%= if @email_log.campaign_id do %>
                    <div class="border-t pt-4 mt-4">
                      ...
                      <div class="flex items-center gap-2 mt-1">
                        <span class="badge badge-secondary badge-sm">{@email_log.campaign_id}</span>
                        <button
                          phx-click="view_related"
              ...
              <div class="card bg-base-100 shadow-sm">
                <div class="card-body">
                  ...
                  <%!-- Body Preview --%>
                  <div class="mb-4">
                    <label class="text-sm font-medium text-gray-600 block mb-2">Body Preview</label>
                  ...
                  <%= if assigns[:show_body] and @email_log.body_full do %>
                    <div class="mb-4">
                      <label class="text-sm font-medium text-gray-600 block mb-2">Full Body</label>
                  ...
                  <%= if assigns[:show_headers] and map_size(@email_log.headers) > 0 do %>
                    <div>
                      <label class="text-sm font-medium text-gray-600 block mb-2">Headers</label>
                      <div class="bg-gray-50 p-3 rounded text-sm max-h-40 overflow-y-auto">
                ...
                <div class="card-body">
                  ...
                  <% else %>
                    <div class="text-center py-8 text-gray-500">
                      <.icon name="hero-clock" class="w-12 h-12 mx-auto mb-2 text-gray-300" />
                  ...
                  <div class="stats stats-vertical shadow-none bg-transparent">
                    ...
                    <%= if @email_log.retry_count > 0 do %>
                      <div class="stat px-0 py-2">
                        ...
                        <div class="stat-value text-lg text-warning">{@email_log.retry_count}</div>
                      </div>
                    <% end %>
                  </div>
                </div>
                  ...
                  <div class="card-body">
                    ...
                    <div class="space-y-2">
                      <%= for related <- Enum.take(@related_emails, 5) do %>
                        <.link
                          navigate={~p"/phoenix_kit/admin/email-logs/#{related.id}"}
                          class="block p-2 rounded hover:bg-gray-50 text-sm"
                        >
                          <div class="font-medium truncate">{related.to}</div>
                          <div class="text-xs text-gray-500 flex items-center justify-between">
                            <span>{UtilsDate.format_datetime_with_user_format(related.sent_at)}</span>
        ...
        <% else %>
          <div class="text-center py-12">
            <.icon name="hero-envelope-open" class="w-16 h-16 mx-auto mb-4 text-gray-300" />
            <h2 class="text-xl font-semibold text-gray-600 mb-2">Email Not Found</h2>
            <p class="text-gray-500 mb-4">The email with ID {@email_id} could not be found.</p>
            <.link navigate={~p"/phoenix_kit/admin/email-logs"} class="btn btn-primary">
              Back to Email Logs
            </.link>
          </div>
        <% end %>
