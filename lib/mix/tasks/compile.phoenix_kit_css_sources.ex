defmodule Mix.Tasks.Compile.PhoenixKitCssSources do
  ...
  def run(_args) do
    ...
    source_lines =
      ...
      |> Enum.uniq()
      |> Enum.sort()
      |> Enum.map(&format_source(&1, deps))

    maybe_warn_zero_sources(source_lines)
  ...
  # A silent empty file is the worst failure mode of CSS auto-discovery: the host's
  # admin UI loses every responsive/variant utility from the dep with no error. When
  # we emit zero @source lines but phoenix_kit-dependent deps are present on disk,
  # surface it loudly so it isn't a multi-hour CSS mystery.
  defp maybe_warn_zero_sources([]) do
    self = Mix.Project.config()[:app]

    case PhoenixKit.ModuleDiscovery.phoenix_kit_dependent_apps() -- [self, :phoenix_kit] do
      [] ->
        :ok

      apps ->
        Mix.shell().error(
          "[PhoenixKit] #{@generated_file} generated with 0 @source lines, but found " <>
            "phoenix_kit-dependent dep(s): #{inspect(apps)}. Their Tailwind classes will " <>
            "NOT be compiled — responsive/variant utilities used only inside those deps " <>
            "will be missing from the bundle. Ensure each module implements css_sources/0, " <>
            "or set `config :phoenix_kit, modules: [...]` as a fallback."
        )
    end
  end

  defp maybe_warn_zero_sources(_), do: :ok
  ...
  # Atom entries — resolved via parent app's mix.exs deps.
  defp format_source(entry, deps) when is_atom(entry) do
    app_name = to_string(entry)

    case find_dep_path(entry, deps) do
