...
### Host `:layout` invoked with `@inner_content`, not `@inner_block` (Phoenix 1.8 layout ergonomics)

`use PhoenixKitWeb, :live_view` sets `layout: PhoenixKit.LayoutConfig.get_layout()` (`phoenix_kit_web.ex:51`), so Phoenix invokes the host's configured `config :phoenix_kit, layout:` function **directly as a LiveView `:layout`** — always with `@inner_content` (a `%Phoenix.LiveView.Rendered{}`), never an `@inner_block` slot. A host layout written in the Phoenix 1.8 function-component idiom (`slot :inner_block` + `render_slot(@inner_block)`) then crashes with `KeyError: key :inner_block not found`, raised inside the host's own layout, on every phoenix_kit auth page (login/register/reset). Our own `app_layout` already tolerates both via `convert_inner_content_to_block/1` (`layout_wrapper.ex`), but a host layout used directly as the `:layout` gets no such grace. Documented contract is `{@inner_content}` (`integration.ex`, "Layout Templates"), but the failure is an opaque `KeyError` far from that doc.

Options: (a) point the LiveView `:layout` at a phoenix_kit-owned wrapper that receives `@inner_content` from Phoenix (guaranteed), synthesizes an `inner_block` slot, and delegates to the host layout via the existing `render_modern_parent_layout` path — makes both conventions work; **higher blast radius** (touches layout resolution for every phoenix_kit LiveView across admin/dashboard/auth and both Phoenix 1.7/1.8 hosts — needs cross-host testing and its own release); or (b) low-effort: detect a slot-style host layout and raise a friendly, actionable error naming the `{@inner_content}` contract instead of the bare `KeyError`.

Surfaced 2026-07-05 by a downstream `langust` report against 1.7.173 (`dev_docs/2026-07-05-phoenix-kit-1.7.173-inner-block-*.md`). Not a 1.7.173 regression — the layout resolution path is unchanged since well before it (confirmed: host fixed it by making their `frontend/1` handle both conventions, no phoenix_kit change). Non-blocking; the consumer explicitly confirmed a guardrail would be nice-to-have, not required.
